export class Fortnight {
  ...
  constructor(index: number) {
    ...
    this.startDate = new Date(
      baseDate.setDate(baseDate.getDate() + (index - 1) * 14)
    );
  ...
  static fromDate(givenDate: Date): Fortnight {
    ...
    const deltaDays = Math.floor(
      (givenDate.getTime() - baseStartDate.getTime()) / millisecondsPerDay
    );
  ...
  static fromRange(start: Date, end: Date): Fortnight[] {
    ...
    while (current <= end) {
      ...
      const alreadyAdded = fortnights.some(
        (fn) => fn.index === fortnight.index
      );
      if (!alreadyAdded) {
