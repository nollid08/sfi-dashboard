...
export class QoutaInfo {
  ...
  constructor({
    currentQoutaPercentage,
    projectedQoutaPercentage,
    currentFortnightlyQoutaPercentages,
    projectedFortnightlyQoutaPercentages,
  }: {
    currentQoutaPercentage: number;
    projectedQoutaPercentage: number;
    currentFortnightlyQoutaPercentages: FortnightlyQoutaPercentage[];
    projectedFortnightlyQoutaPercentages: FortnightlyQoutaPercentage[];
  }) {
    ...
    this.currentFortnightlyQoutaPercentages =
      currentFortnightlyQoutaPercentages;
    this.projectedFortnightlyQoutaPercentages =
      projectedFortnightlyQoutaPercentages;
  }

  static async buildProjectedQoutaInfo(
    newSessions: BookingSession[],
    ...
    const fortnights: Fortnight[] = Fortnight.fromRange(
      earliestSessionTime,
      latestSessionTime
    );
    ...
    const sessionsRef = await db
      .collection("sessions")
      .where("coaches", "array-contains", coach.uid)
      .where("arrivalTime", ">=", fortnights[0].startDate)
      .where("leaveTime", "<=", fortnights[fortnights.length - 1].endDate)
      .get();
    ...
    for (const fortnight of fortnights) {
      const sessionsInFortnight = sessions.filter(
        (session) =>
          session.arrivalTime >= fortnight.startDate &&
          session.endTime <= fortnight.endDate
      );
      const newSessionsInFortnight = newSessions.filter(
        (session) =>
          session.arrivalTime >= fortnight.startDate &&
          session.endTime <= fortnight.endDate
      );
      const projectedSessionsInFortnight = sessionsInFortnight.concat(
        newSessionsInFortnight
      );
      ...
      currentFortnightlyQoutaPercentages.push(
        new FortnightlyQoutaPercentage({
          fortnight,
      ...
      let projectedTimeCoveredInFortnight = projectedSessionsInFortnight.reduce(
        (acc, session) => {
          const totalsessionTime =
            session.endTime.getTime() - session.startTime.getTime();
      ...
      projectedPercentageCovered =
        projectedTimeCoveredInFortnight / currentCoachQouta;
      projectedFortnightlyQoutaPercentages.push(
        new FortnightlyQoutaPercentage({
          fortnight,
          qoutaPercentage: projectedPercentageCovered,
        })
      );
    }
