...
export const find_sessions_available_coaches_function = onCall(
  { secrets: [mapsApiKey] },
  async (request: CallableRequest) => {
    console.log("Finding Available Coaches");
    const sessionData: {
      id: string;
      arrivalTime: number;
      leaveTime: number;
    ...
    const session = new BookingSession({
      ...
      activity: new Activity(
        sessionData.activity.id,
        sessionData.activity.name,
        sessionData.activity.icon,
        sessionData.activity.color
      ),
      ...
      client: new Client({
        ...
        clientType: new ClientType(
          sessionData.client.type.id,
          sessionData.client.type.name
        ),
      }),
    });
    ...
    const coachesWhoCoverActivityRef = db
      .collection("users")
      .where("activitiesCovered", "array-contains", session.activity.id);
    ...
    for (const leave of leaves) {
      if (leave.status != LeaveStatus.rejected) {
        if (
          leave.startDate <= session.arrivalTime &&
          leave.endDate >= session.endTime
        ) {
          onLeaveCoaches.push(
        ...
        if (
          leave.startDate >= session.arrivalTime &&
          leave.startDate <= session.endTime
        ) {
          onLeaveCoaches.push(
            notWorkingCoachesWhoCoverActivity.find(
              (coach) => coach.uid === leave.coachUid
        ...
        if (
          leave.endDate >= session.arrivalTime &&
          leave.endDate <= session.endTime
        ) {
          onLeaveCoaches.push(
            notWorkingCoachesWhoCoverActivity.find(
    ...
    const availableCoachesWhoCoverActivity =
